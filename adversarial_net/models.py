...
flags.add_argument(scope="adv_cl_inputs", name="phase", argtype=str, default="train")
...
flags.add_argument(scope="adv_cl_loss", name="adv_reg_coeff", argtype=float, default=1)
...
class BaseModel(object):
    ...
    def _fit(self, model_inpus = None, save_model_path = None, pretrained_model_path = None, variables_to_restore = None):
        ...
        feed_dict = None
        if model_inpus:
            feed_dict = zip(self.inputs, model_inpus)
    ...
    def maybe_restore_pretrained_model(self, sess, saver_for_restore, model_dir, train_dir):
        ...
        if not (pretrain_ckpt and pretrain_ckpt.model_checkpoint_path):
            logger.info('Asked to restore model from %s but no checkpoint found.' % model_dir)
            return
    ...
    def run_training(self, train_op, loss, acc=None, feed_dict=None, save_model_path=None, variables_to_restore=None,
        ...
        with tf.Session() as sess:
            ...
            if self.arguments["should_restore_if_could"] and save_model_path is not None:
                ...
                model_checkpoint_exists = ckpt and ckpt.model_checkpoint_path
...
class AdversarialClassificationModel(BaseModel):
    ...
    def build(self):
        ...
        self.loss = self.cl_loss + self.adv_loss * tf.constant(self.arguments["avd_cl_loss"]["adv_reg_coeff"], name='adv_reg_coeff')
    ...
    def get_cl_loss(self, lstm_output_tensor, y_tensor, weight_tensor):
        final_output_tensor = tf.gather(lstm_output_tensor, [-1], axis=1)
        final_output_weights = tf.squeeze(tf.gather(weight_tensor, [-1], axis=1))
    ...
    def get_adversarial_loss(self, y_tensor, weight_tensor):
        embedding_grads = tf.gradients(self.cl_loss, self.sequences["lm_sequence"].embdedding)
        ...
        lstm_initial_state = self.get_lstm_state()
        sequence_len = self.inputs.length
        perturbed_embedding = self.sequences["lm_sequence"].embdedding + perturb
        lstm_output_tensor = self.sequences["lm_sequence"].lstm_layer(perturbed_embedding, lstm_initial_state, sequence_len)
